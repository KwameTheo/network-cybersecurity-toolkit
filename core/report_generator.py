...
def export_report_txt(report_data: Dict[str, Any], output_path: Optional[str] = None) -> str:
    ...
    lines.append(f"  Author & Creator: {meta.get('author', AUTHOR_NAME)} | {meta.get('copyright', COPYRIGHT_NOTICE)}")
    lines.append(f"  Generated: {meta.get('generated_at', '')}  |  Report ID: #{meta.get('report_id', '--')}")
    lines.append(f"  Cryptographic Provenance Seal: {meta.get('provenance_seal', '')[:24]}...")
    lines.append(sep)
    lines.append("")
    ...
    lines.append(sep)
    lines.append(f"  END OF AUDIT REPORT -- {PRODUCT_NAME}")
    lines.append(f"  {COPYRIGHT_NOTICE} | Author: {AUTHOR_NAME}")
    lines.append("  All Rights Reserved. Originality Certified by Cryptographic Provenance.")
    lines.append(sep)
...
def export_report_pdf(report_data: Dict[str, Any], output_path: Optional[str] = None) -> str:
    ...
    if sec_audit:
        ...
        rating = sec_audit.get("overall_posture") or sec_audit.get("posture_rating", "UNKNOWN")
        passed = sec_audit.get("passed_count", sec_audit.get("passed_checks", 0))
    ...
    # Provenance watermark & legal copyright footer
    seal = meta.get("provenance_seal", "")
    story.append(Paragraph(
        f"<font size=7 color='#6B7280'><b>{PRODUCT_NAME} (v{PRODUCT_VERSION})</b> | Author & Sole Creator: <b>{AUTHOR_NAME}</b> | {COPYRIGHT_NOTICE}<br/>Cryptographic Provenance Seal: {seal}</font>",
        body_style
    ))
